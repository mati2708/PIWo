    ...
    const saveGame = async (gameData) => {
        try {
            ...
            if (existingGame) {
                ...
                // --- NAPRAWA NA 5.0 ---
                const newId = Date.now(); // Generujemy unikalne ID (np. 1715839201923)
                
                const newGame = {
                    ...gameData,
                    id: newId, // <-- PRZYPISUJEMY ID DO DOKUMENTU!
                    ownerUid: user ? user.uid : "anonim",
                    ownerEmail: user ? user.email : "Brak",
                    isSold: false
                };
                
                // Zapisujemy grę tak, by nazwa dokumentu z lewej strony była taka sama jak jej ID
                await setDoc(doc(db, "games", newId.toString()), newGame);
            }
            await fetchGames();
        } catch (error) {
