...
import { useAuth } from '@/context/AuthContext'; // <-- Dodany import logowania

export default function Home() {
    // customowy hook do pobierania gier z API/LocalStorage
    const { games, loading, buyGame } = useGames();
    // hook Next.js do nawigacji pomiędzy podstronami
    const router = useRouter();
    // Wyciągamy dane użytkownika i funkcję wylogowywania
    const { user, logout } = useAuth(); // <-- Dodany hook logowania
        ...
        <div className="min-h-screen bg-[#f8f9fa] text-gray-800 font-sans">
            ...
            <header className="bg-[#2c3e50] text-white py-4 px-6 md:px-10 flex justify-between items-center flex-wrap gap-4 shadow-md">
                ...
                <div className="flex gap-3 text-sm md:text-base items-center">
                    ...
                    {/* --- INTELIGENTNY PRZYCISK LOGOWANIA --- */}
                    {user ? (
                        <button 
                            onClick={logout} 
                    ...
                    {filteredGames.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            ...
                            {currentGames.map(game => {
                                ...
                                // Sprawdzamy, czy gra jest sprzedana
                                const isSold = game.isSold;
                                
                                return (
                                    <div 
                                        key={game.id} 
                                        onClick={() => !isSold && router.push(`/game/${game.id}`)} // Blokada wejścia w szczegóły, jeśli sprzedana
                                        // Jeśli isSold, dodajemy klasy wyszarzające i wyłączamy kursor pointer
                                    ...
                                    >
                                        ...
                                        <div className="flex gap-2 mt-auto">
                                            <button 
                                                ...
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    buyGame(game.id); // <-- WYWOŁANIE ZAKUPU
                                                }}
